...
class GeneticImage {
    ...
    GeneticImage(NSImage*itarget) {
        ...
        target=(uint32_t*)[ImageHelper convertImageToBitmapRGBA8:itarget];
        imgBuffer=[ImageBuffer initWithWidth:w Height:h];
        bmp=imgBuffer.imgBuff32;
    ...
    void generateMetal() {
        uint *scoresTmp=(uint*)calloc(nOrgs, sizeof(*scoresTmp));

        generateGenePool(); // -> genePool of maxGenePerOrg
        
        [md compileFunc:@"geneticAlgo"]; // generate cellScores(w,h,nOrg)
        
        id<MTLBuffer>targetBuff=[md createBuffer:target length:sizeBytes],
        
        genesBuff    =[md createBuffer:genePool.data()  length:genePool.size()*sizeof(Gene)],
        scoresTmpBuff=[md createBuffer:scoresTmp        length:nOrgs * sizeof(*scoresTmp)];
        ...
        [md setBufferParam:scoresTmpBuff  index:3]; // -> sum by organism (sum(w*h))
        [md setIntParam:&w                index:4];
        
        [md runThreads3d:w height:h depth:nOrgs];    // w x h x nOrg grid -> cellSCores
        
        [md copyContentsOn:scoresTmp buffer:scoresTmpBuff];
        
        float *scores=new float[nOrgs]; // -> float scores from uit scoresTmp
        for (int i=0; i<nOrgs; i++) scores[i] = adjustScore(scoresTmp[i]);
