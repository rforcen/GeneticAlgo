...
-(id<MTLBuffer>)createGPUPrivateBuffer: (void*)data length:(NSInteger)len {
    return [_device newBufferWithBytes:data  length:len options:MTLResourceStorageModePrivate];
}
...
-(void)setIntParam:(void*)data  index:(int)index {
    [_commandEncoder setBytes:data length:sizeof(int) atIndex:index];
}
-(void)setFloatParam:(void*)data  index:(int)index {
    [_commandEncoder setBytes:data length:sizeof(float) atIndex:index];
}
...
-(void)compileFunc:(NSString*)func {
    ...
    _maxMem = [_pipeline staticThreadgroupMemoryLength];
