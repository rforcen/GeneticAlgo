...
class Organism {
    ...
    float scoreIt(color*target, int w, int h) {
        int size=w*h;
        score=0;
...
class GeneticImage {
    ...
    GeneticImage(NSImage*itarget) {
        ...
        bmp     =(uint32_t*)[md newData: sizeBytes]; // local working copy must memcpy to imgBuffer to display
        cls();
    ...
    float scaleScore(float s) { return parent.scaleScore(s, size); }
    ...
    void generateMetal() {
        ...
        for (int i=0; i<nOrgs; i++) scores[i] = scaleScore(scores_uint[i]);
    ...
    float scoreImageMetal() {
        uint sc=0;
        uint szScore=1*sizeof(uint), *score=(uint*)[md newData:szScore];
        id<MTLBuffer>scoreBuff = [md copyBuffer:score  length:szScore];
        
        [md compileFunc:@"scoreImage"];
        
        [md setBufferParam:bmpBuff      index:0];
        [md setBufferParam:targetBuff   index:1];
        [md setIntParam:&w              index:2];
        [md setBufferParam:scoreBuff    index:3];
        ...
        return parent.score = scaleScore(sc);
    }
